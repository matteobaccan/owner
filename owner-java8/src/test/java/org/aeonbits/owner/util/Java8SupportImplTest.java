...
/**
 * Unit test for {@link Java8SupportImpl}, the implementation loaded reflectively by
 * {@link Reflection} to support interface default methods.
 * <p>
 * It exercises both invocation strategies of
 * {@link Java8SupportImpl#invokeDefaultMethod(Object, Method, Object[])}: the
 * {@code MethodHandles.lookup().findSpecial()} path used on Java 9+, and the private
 * {@code MethodHandles.Lookup(Class, int)} constructor hack used on Java 8, which is forced
 * here by setting the internal {@code isJava8} flag via reflection.
 *
 * @author Matteo Baccan
 */
public class Java8SupportImplTest {
    ...
    public void isDefaultShouldReturnTrueForInterfaceDefaultMethods() throws Exception {
        assertTrue(new Java8SupportImpl().isDefault(sumMethod()));
    }

    @Test
    public void isDefaultShouldReturnFalseForAbstractInterfaceMethods() throws Exception {
        assertFalse(new Java8SupportImpl().isDefault(Sample.class.getMethod("three")));
    }
    ...
    public void invokeDefaultMethodShouldInvokeDefaultImplementationBypassingTheProxyHandler() throws Throwable {
        ...
        Java8SupportImpl support = newSupport(false);

        Object result = support.invokeDefaultMethod(newProxy(), sumMethod(), new Object[] {2, 3});
    ...
    public void invokeDefaultMethodShouldUseTheLookupConstructorHackWhenRunningOnJava8() throws Throwable {
        Java8SupportImpl support = newSupport(true);
        ...
        if (isJava8Runtime()) {
            ...
            Object result = support.invokeDefaultMethod(newProxy(), sumMethod(), new Object[] {3, 4});
            assertEquals(Integer.valueOf(7), result);
        } else {
            try {
                support.invokeDefaultMethod(newProxy(), sumMethod(), new Object[] {3, 4});
                ...
                // Java8SupportImpl.Lookup.lookupConstructor() returns null.
    ...
    /**
     * Creates a {@link Java8SupportImpl} forcing its private {@code isJava8} flag, so both
     * branches of {@code invokeDefaultMethod} can be exercised on any JVM.
     */
    private static Java8SupportImpl newSupport(boolean isJava8) throws Exception {
        Java8SupportImpl support = new Java8SupportImpl();
        Field field = Java8SupportImpl.class.getDeclaredField("isJava8");
        field.setAccessible(true);
        field.setBoolean(support, isJava8);
        return support;
    }
    ...
    private static Sample newProxy() {
        return (Sample) Proxy.newProxyInstance(
                Java8SupportImplTest.class.getClassLoader(),
     ...
     * performed by the {@link Java8SupportImpl} constructor.
     */
