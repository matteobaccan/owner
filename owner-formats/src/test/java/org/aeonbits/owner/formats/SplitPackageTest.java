 ...
 * <code>ZooKeeperLoader</code> came to sit in <code>org.aeonbits.owner.loaders</code> for eleven years, in
 * the artifact next door; this artifact is younger and starts with the test rather than acquiring it.
 * </p>
 * <p>
 * The temptation here is specific and worth naming: a loader is an implementation of an interface that
 * lives in <code>org.aeonbits.owner.loaders</code>, and putting it beside that interface reads as tidy
 * right up to the moment somebody puts both jars on the module path.
 * </p>
 *
...
public class SplitPackageTest {
    ...
    @Test
    public void theFormatsArtifactSharesNoPackageWithTheCore() throws IOException, URISyntaxException {
        Set<String> core = packagesOf(Config.class);
        Set<String> formats = packagesOf(JsonLoader.class);

        assertFalse("no package found in the core artifact", core.isEmpty());
        assertFalse("no package found in the formats artifact", formats.isEmpty());

        Set<String> shared = new TreeSet<>(formats);
     ...
     */
    @Test
    public void theJsonLoaderStaysOutOfThePackageOwnedByTheCore() throws IOException, URISyntaxException {
        assertEquals("org.aeonbits.owner.formats.json", JsonLoader.class.getPackage().getName());
        ...
        assertFalse("the formats artifact should not put anything in it",
                packagesOf(JsonLoader.class).contains("org.aeonbits.owner.loaders"));
    }
