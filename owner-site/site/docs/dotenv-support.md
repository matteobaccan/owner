...
permalink: /docs/dotenv-support/
---

A `.env` file is a list of `NAME=value` lines, and it is how container tooling carries configuration into a
process: `docker run --env-file`, `env_file` in Compose, `envFrom` in Kubernetes, the secrets of a CI
pipeline. OWNER reads one directly, out of the box, with no dependency to add.
...
Any source whose path ends in `.env` goes to `DotEnvLoader`, which covers both a file that is all
extension and one named `staging.env`. Values arrive as text and are converted by the usual
[type conversion]({{ site.url }}/docs/type-conversion/) rules, so `port()` is an `int` and `timeout()` a
`Duration` exactly as they would be from a properties file. A `.env` also merges with other sources under
...
Note that no dialect interpolates `${...}`: OWNER expands variables in property values
[itself]({{ site.url }}/docs/variables-expansion/), after loading and across every source, so a loader
doing its own would expand them twice.
...
real fix, and the other three are for when reading them verbatim is what you want.

**In a `logging.properties` file**, which is how a JDK application is usually configured:

```properties
org.aeonbits.owner.loaders.DotEnvLoader.level = OFF
```

pointed at with `-Djava.util.logging.config.file=/path/to/logging.properties`. Logger names are a
hierarchy, so `org.aeonbits.owner.level = OFF` silences the whole library at once.
...
`jul-to-slf4j`, or into Log4j 2 with its JUL adapter, the message arrives in the ordinary logs and is
silenced by the ordinary configuration, under the logger name
`org.aeonbits.owner.loaders.DotEnvLoader`.

This is one of only three things OWNER ever says. The others are a hot reload that failed — reported once,
because the configuration keeps the values it had and tries again at the next check — and an XML parser
that does not support one of the hardening features, which is worth knowing because the protection is then
not in force. All three sit under `org.aeonbits.owner`, so
`org.aeonbits.owner.level = OFF` turns off every one of them at a stroke.


  [loader]: https://matteobaccan.github.io/owner/apidocs/latest/org/aeonbits/owner/loaders/DotEnvLoader.html
